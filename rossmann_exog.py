"""Rossmann sweep with exogenous regressors (Promo, StateHoliday, SchoolHoliday, Open).

Self-contained variant of the main CL-Occam pipeline that wires the
Kaggle Rossmann exogenous columns into the ETNA TSDataset.  Gradient
boosters and Ridge variants receive the holiday/promo signal as
known-future regressors.  Naive-7 and ETS ignore exog by construction.

Method order and hyperparameters match the main experiment to keep the
Rossmann-with-exog results directly comparable to the no-exog baseline.
"""
...
ROSSMANN_RAW_PATH = os.path.join("data", "rossman", "train.csv")
...
def _cl_occam_clustering(
    ...
    """Run DTW clustering + per-cluster CV-Occam selection on sales data only.
