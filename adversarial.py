"""
Adversarial random-cluster ablation for the CL-Occam thesis.

For each backtest window of ROSSMANN at horizons in {7, 14, 30}, the
DTW-derived partition and per-cluster model mapping are read from
``results/seed_ROSSMANN.json``.  The same `forecast_clustered_local`
...
Outputs:
    results/adversarial_ROSSMANN.csv  per (horizon, seed) summary
...
from run_experiment import _evaluate_window_forecasts


RESULTS_DIR = "results"
SEED_PATH = os.path.join(RESULTS_DIR, "seed_ROSSMANN.json")
R = 10
...
def main() -> int:
    ...
    for horizon_str, h_block in seed["horizons"].items():
        ...
        for seed_idx in range(R):
            ...
            for w in windows:
                ...
                # choices but reassign by random cluster id so a 'Ridge' slot
                # does not align with the same DTW cluster id across seeds.
    ...
    pd.DataFrame(summary_rows).to_csv(os.path.join(RESULTS_DIR, "adversarial_ROSSMANN.csv"), index=False)
    pd.DataFrame(paired_rows).to_csv(os.path.join(RESULTS_DIR, "adversarial_paired.csv"), index=False)
    print("\nWrote results/adversarial_ROSSMANN.csv and results/adversarial_paired.csv")
    return 0
