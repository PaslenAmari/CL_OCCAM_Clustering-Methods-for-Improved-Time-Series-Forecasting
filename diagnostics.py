"""
Per-cluster diagnostics and computational cost figures for the CL-Occam thesis.
...
Writes:
    ...
    results/figures/fig_centroids.pdf         z-scored cluster centroids
...
def figure_per_cluster_mae(df: pd.DataFrame) -> None:
    ...
    for ax, h in zip(axes, horizons):
        sub = df[df["horizon"] == h]
        # Average across windows for each cluster id (informative summary).
...
def figure_dtw_scaling(X: np.ndarray) -> pd.DataFrame:
    ...
    # Fit a log-log slope: time ∝ N^b → log(time) = log(a) + b·log(N)
