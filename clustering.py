"""
Clustering of heterogeneous time series for the CL-Occam clustered-local
forecasting pipeline.

The default algorithm is DTW + Agglomerative Clustering with average
linkage over z-score-normalised series.  Two design decisions relative
to a naive DTW+KMeans baseline:

* The number of clusters ``k`` is selected by a validation curve over
  the most recent ``h`` days of training data, replacing the simpler
...
try:
    ...
    # contains an indexing bug on Python >= 3.13.
...
def dtw_distance_matrix(X: np.ndarray) -> np.ndarray:
    ...
    Uses a serial loop over ``distance_fast`` rather than the parallel
    C-extension path (``distance_matrix_fast(parallel=True)``): the
    parallel path interacts badly with ETNA/pandas threading on Windows
    and causes intermittent access-violation crashes inside
    ``pandas.concat``.  The serial loop is slower for the DTW step alone
    but that step is a small fraction of total wall-clock, and the
    stability improvement outweighs the cost.
    """
...
def select_k_by_validation(
    ...
    if best_labels is None:
        # Defensive fallback if every candidate produced NaN.
