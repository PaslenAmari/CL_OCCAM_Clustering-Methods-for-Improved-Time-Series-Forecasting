"""
Worldwide Electricity Load — Clustering and Forecasting (CL-Occam Thesis).

Primary dataset for full CL-Occam pipeline evaluation with Automated
Occam's Razor. Compares a unified global model against a cluster-ensemble
approach on a highly heterogeneous daily electricity load panel.
"""
...
def get_electricity_data():
    ...
    # Chronological window for cross-sectional alignment
...
def cluster_dtw(X, n_clusters=4):
    ...
    # Fall back to KMeans if Agglomerative produces a single dominant cluster.
    counts = np.bincount(labels)
    if counts.max() / counts.sum() > 0.85 and n >= 4:
        labels = KMeans(n_clusters=n_clusters, random_state=42).fit_predict(D)
        sizes = dict(zip(*np.unique(labels, return_counts=True)))
        print(f"  Cluster sizes after KMeans fallback: {sizes}")
...
def select_model_for_cluster_via_cv(X_train, cluster_mask, horizon, cluster_id, start_date):
    ...
    # Automated Occam's Razor: prefer the simpler linear model when its
    # cross-validation MAE is within a tolerance margin of the best model.
    # A smaller cluster has a wider margin because less training data
    # increases overfitting risk for the more complex model.
...
def build_cluster_models(X_train, cluster_labels, horizon, model_mapping, start_date):
    """Train a per-cluster ETNA pipeline using the model class from model_mapping."""
    print("CLUSTER MODELS: Building per-cluster models...")
    cluster_forecasts = {}
...
def main():
    ...
    # Forecast horizon and chronological backtest windows
    horizon = 30
    n_windows = 4
    ...
    # Determine number of clusters based on panel size
    ...
    # Wilcoxon Signed-Rank Test: non-parametric paired test robust to
    # non-normal error distributions across regions.
