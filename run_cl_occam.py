"""
Main experiment runner for the CL-Occam clustered-local forecasting pipeline.

Sweeps two real-world daily panels (ELEC, ROSSMANN) over three forecast horizons
({7, 14, 30}) and six methods (5 baselines + CL-Occam), produces:
    ...
    results/main_paired_tests.csv    CL-Occam vs each baseline, Holm-corrected
                                     across the 30-test family
...
HORIZONS = (7, 14, 30)
N_WINDOWS = 4
DATASETS = ("ELEC", "ROSSMANN")
# Set to {"local_catboost"} to skip the per-series local CatBoost run
# and reproduce a smaller-budget variant of the experiment.
SKIP_METHODS: set[str] = set()
# k-grid for prepare_clustering; values beyond 6 add wall-clock with
# negligible expected MAE gain on the NN5 and ELEC panels.
K_GRID = (3, 4, 5, 6)
...
def _cluster_score_fn(horizon: int):
    ...
    def score(inner_train: np.ndarray, inner_val: np.ndarray, labels: np.ndarray) -> float:
        # Use CatBoost for scoring inside k selection (cheap and consistent).
        # Using full CL-Occam selection inside k selection would be circular
        # and ~10x slower without a meaningful payoff.
        try:
...
def run_one_cell(
    ...
    for window_idx, (test_start, test_end) in enumerate(test_windows):
        ...
        # Re-run clustering and selection on the current training fold so each
        # window uses only past information (no look-ahead).
        ...
        for method in registry:
            ...
            # Release memory between methods to reduce peak usage.
            del forecasts
            gc.collect()
...
def run_all(seed: int = 42) -> None:
    ...
    for dataset_name in DATASETS:
        ...
        for horizon in HORIZONS:
            ...
            # Incremental save — protects against mid-run failure on a long sweep.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="CL-Occam main experiment runner")
