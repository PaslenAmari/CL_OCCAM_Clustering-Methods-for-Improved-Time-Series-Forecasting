"""
Ablation studies for the CL-Occam thesis.

Each ablation isolates one design decision while keeping the rest of
the pipeline frozen, and writes a CSV corresponding to one results table.
All ablations run on a single horizon ``ABLATION_HORIZON`` to keep
wall-clock tractable; the main results table covers the full horizon sweep.
...
def build_partition(
    ...
    if algorithm == "dtw_agglomerative":
        if signature_name != "raw":
            # DTW is only meaningful on time-aligned raw series; for derived
            # signatures fall back to Euclidean to keep the ablation matrix
            # well-defined.
...
def ablation_tau(
    ...
    """Sweep the Occam tolerance threshold tau.

    Small and large clusters share the same tau value here to keep the
    sensitivity curve one-dimensional.
    """
...
def main() -> int:
    """Run all ablations on ELEC and ROSSMANN; append results to CSV files."""
    os.makedirs(RESULTS_DIR, exist_ok=True)
    for dataset_name in ("ELEC", "ROSSMANN"):
