"""
Generate thesis figures from results CSVs.
...
def figure_horizon_effect(paired_csv: str = None) -> None:
    ...
    Visualises the horizon-dependence of the CL-Occam advantage: the benefit
    should be larger on longer horizons where a global model averages away
    regime structure in a heterogeneous panel.
    """
...
def figure_rossmann_cluster_metadata() -> None:
    ...
    Shows that the DTW partition (computed on z-scored shapes only)
    aligns with store-type metadata it never observed — one row per
    cluster, one column per StoreType, cell value = number of stores.
    """
    try:
...
def figure_improvement_vs_global_cb(metrics: pd.DataFrame) -> None:
    ...
    ordered by within-panel shape heterogeneity.
    """
    if metrics.empty:
        return
...
def main() -> int:
    """Generate all thesis figures from results CSVs."""
