"""
PowerCons Clustering Benchmark — CL-Occam Thesis, Paradigm Comparison.

Three clustering approaches are evaluated on the UCR PowerCons dataset
(or a synthetic fallback) before being integrated with ETNA forecasting:
    Approach 1 — TSFresh statistical features + KMeans (evaluated, discarded)
    Approach 2 — DTW distance matrix + KMeans (primary CL-Occam approach)
    Approach 3 — sktime TimeSeriesKMeans with DTW metric (evaluated, discarded)

Fedot.Industrial is used as an AutoML model selector per cluster; its
recommendation is mapped to either CatBoost or a linear model in ETNA.
"""
...
# sktime (isolated virtual environment; evaluated as an alternative to DTW agglomerative)
SKTIME_VENV_PATH = os.environ.get("SKTIME_VENV_PATH", "/opt/sktime_env/bin/python")

# Fallback for Docker environments without a dedicated sktime virtualenv
...
# Fedot.Industrial (isolated virtual environment; used as AutoML model selector per cluster)
FEDOT_VENV_PATH = os.environ.get("FEDOT_VENV_PATH", "/opt/fedot_env/bin/python")

# Fallback for Docker environments without a dedicated Fedot virtualenv
if not os.path.exists(FEDOT_VENV_PATH):
    import sys
    FEDOT_VENV_PATH = sys.executable

try:
    # Mock the renamed metrics repository module before importing Fedot.Industrial
    # to avoid ImportError on fedot >= 0.7 where that submodule was relocated.
    mock_script = """
import sys
try:
...
def load_synthetic_data():
    """Generate a synthetic energy consumption panel as a fallback dataset."""
...
def cluster_tsfresh(X, n_clusters=4):
    """
    Approach 1 (TSFresh + KMeans): clustering based on statistical features.

    Steps:
        1. Convert series to long format (id, time, value).
        2. Extract numeric summary statistics with TSFresh.
        3. Reduce dimensionality with PCA (99% variance).
        4. Apply KMeans on the feature matrix.

    Evaluated as an alternative to DTW; discarded because DTW produced
    superior Silhouette and ARI scores on PowerCons.
    """
...
def cluster_dtw(X, n_clusters=4):
    """
    Approach 2 (DTW + KMeans): clustering with Dynamic Time Warping distances.

    Steps:
        1. Z-score normalise each series so clustering reflects shape, not scale.
        2. Build a pairwise DTW distance matrix (subsampled to 50 series for speed).
        3. Apply KMeans on the distance matrix.

    This is the primary clustering approach adopted by CL-Occam.
    """
...
def cluster_sktime(X, n_clusters=4):
    """
    Approach 3 (sktime TimeSeriesKMeans with DTW metric).

    Runs inside an isolated virtual environment via subprocess to avoid
    dependency conflicts with the main ETNA environment.
    Evaluated as an alternative; discarded in favour of DTW agglomerative.
    """
...
def build_global_model(X_train, horizon=24):
    """
    Global CatBoost model trained jointly on all segments simultaneously.
    Serves as the performance baseline for the cluster-ensemble comparison.
    """
    ...
    try:
        ...
        print(f"  Training data: min={X_train.min():.2f}, max={X_train.max():.2f}, mean={X_train.mean():.2f}")
...
def build_cluster_models(X_train, cluster_labels, horizon=24, fedot_model_mapping=None):
    """
    Train one ETNA pipeline per cluster using the model class from fedot_model_mapping.

    If fedot_model_mapping is None or a cluster is missing, CatBoost is used
    as the default model class.
    """
    if not ETNA_AVAILABLE:
        print("[ERROR] ETNA not installed!")
        return {}
    ...
    try:
        for cluster_id in np.unique(cluster_labels):
            ...
            # Apply the same feature engineering stack as the global model
...
def aggregate_cluster_forecasts(cluster_labels, cluster_forecasts):
    """Assemble the per-cluster forecasts into a single ensemble forecast dict."""
    ensemble = {}
...
def evaluate_forecasts(X_val, global_forecasts, ensemble_forecasts, horizon, fedot_forecasts=None):
    """Compute MAE, RMSE, and MAPE for global and ensemble models."""
    print("\nEVALUATION: Global Model vs Cluster-Ensemble")

    if not global_forecasts:
        print("  [!] global_forecasts is empty.")
    if not ensemble_forecasts:
        print("  [!] ensemble_forecasts is empty.")
...
def main():
    """Main backtesting pipeline: compares TSFresh, DTW, and sktime clustering approaches."""
    print("\n" + "="*80)
    print("PIPELINE: PowerCons Clustering and Forecasting Benchmark")
    print("="*80)
    ...
    # Six non-overlapping backtest windows covering the second half of the dataset.
    n_total = X.shape[1]
    test_windows = [
    ...
    for approach_name, labels in approaches:
        ...
        for i, (train_end, test_end) in enumerate(test_windows):
            ...
            # Stage I: Fedot selects the best model class per cluster
            fedot_model_mapping = {}
        ...
        # Visualise the final backtest window
