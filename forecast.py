...
for the CL-Occam clustered-local forecasting pipeline.

Two design constraints applied throughout:

* Identical hyperparameters across the global, local, and cluster
  branches whenever a model class is reused.  This isolates the
  clustering and selection effects from regularisation differences
...
* All baselines and the proposed method share the same feature stack
  ...
  differences therefore reflect only the modelling decision under study.
"""
...
"""Single CatBoost hyperparameter set shared by the global, local, and
per-cluster branches.  Set the env var ``CL_OCCAM_CATBOOST_GPU=1`` to
switch to GPU training.  The CPU default uses all available cores
(``thread_count=-1``).

GPU is recommended when the panel has a single heavy global fit per
window (Global CB / CB+ID / CL-Occam with CatBoost clusters) and is
not recommended for ``local_catboost`` where ~N small fits per cell
are dominated by per-fit GPU initialisation overhead."""
...
def make_model(model_type: str):
    ...
    if model_type == "catboost_id":
        ...
        # SegmentEncoderTransform adds a 'segment_code' column (dtype='category');
        # CatBoost auto-detects categorical features from dtype, so
        # cat_features does not need to be passed explicitly.
        return CatBoostMultiSegmentModel(**CATBOOST_PARAMS)
    if model_type == "ridge":
    ...
    if model_type == "ridge_id":
        # Ridge analogue of the ID baseline: a per-segment target mean column
        # added by MeanSegmentEncoderTransform acts as a series-specific
        # intercept without requiring N-1 extra dummy columns.
        return SklearnMultiSegmentModel(
            regressor=make_pipeline(
                SimpleImputer(strategy="constant", fill_value=0.0),
                Ridge(alpha=RIDGE_ALPHA, random_state=42),
